...
// SEEKLAB: implicit while this file lived in BetterExperience.Features.
using BetterExperience;
using BetterExperience.Features;

// SEEKLAB COPY of BetterExperience.Features/MissionControlFeature.cs. It has to come along: the
// window binds DIRECTLY to an AutoThrustService INSTANCE (InitAutoThrust), and BE's copy resolves
// BE's type, not the lab's. Left behind, every toggle and slider on the panel silently drives a
// service that is no longer running — the controls look normal and do nothing, which is
// indistinguishable from the feature itself being broken.
//
// SEEKLAB: namespace — see AutoSeekerFeature.cs.
namespace SeekLab;
...
internal class MissionControlFeature : PluginFeature
{
	...
	private class MissionControlWindow : DrawableWindow
	{
			...
			: base(560, 390)
		{
			...
			// Free-space characterisation: needs NO penetration and no partner. Sweeps each pelvis
			// axis through its full range and logs the character's own kinematic map.
			...
			// length out, so the hips can fine-tune from a near-neutral pose. Bounded and
			// self-limiting (stops at 4 cm of error, 40 cm total).
			...
			// ANGLE readout: prints the hole axis and the shaft axis side by side so they can be
			// checked against what is on screen. Works with OR without penetration.
			...
			// a log line, which cannot be seen while playing.
		...
		public void Refresh()
		{
			if (atService != null)
			{
				...
				if (strokeAuditToggle.Value != atService.StrokeAudit)
				{
