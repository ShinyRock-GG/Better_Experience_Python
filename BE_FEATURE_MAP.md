...
| LipsDeformer | STATIC — minimal game surface | xref liveness v2 (2026-08-05): LipsDeformer touches exactly **2** game types via call edges — `IIKUpdater` (×2) and `ExtendedMonoBehaviour` (×1). Cross-verified by an independent graph.json query (agrees at 2). | Corrects a stale expectation of 5 types (IIKUpdater, InteraccionDeCharacter, ExtTransform, InteraccionesBasicasDeMale, ExtendedMonoBehaviour): the other 3 have **zero** call/field/subscribe/patch edges AND zero signature mentions from LipsDeformer — they are touched only by other plugins, not LipsDeformer. Its real static coupling is just IIKUpdater + ExtendedMonoBehaviour. Not live-tested; small surface ≠ working — needs a live probe to confirm the deformation actually applies. |
