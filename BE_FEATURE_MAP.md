...
| NaturalLanguage | UI-COLLISION (ensemble); presumed functional standalone | Source: dock = DockingContainer(Vector2Int.down) → bottom-center — the exact region AIChat's opaque chat bar occupies; Enter opens the dock INVISIBLY behind it. Gate (Session.Guest != null, dock closed) otherwise passes | Cross-mod UI collision → conflicts inventory (BE NL dock vs AIChat chat bar, same anchor). Not a public-release bug (masses without AIChat unaffected) — verify in the clean-environment pass. Ensemble fix options: move NL dock anchor, or AIChat repositions, or NL disabled in this install. |
